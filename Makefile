...
.PHONY: build check clean format help test tidy

all: check test build ## Default target: check, test, build

build: ## Build all executables, located under ./bin/
	@echo "Building..."
	@CGO_ENABLED=0 go build -o ./bin -trimpath $(LDFLAGS) ./example/...
	@CGO_ENABLED=0 go build -o ./bin -trimpath $(LDFLAGS) ./cmd/pocketbase/...
